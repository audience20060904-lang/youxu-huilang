...
- **美术的明暗一律是黑白半透明叠在 `currentColor` 上**（暗 `#000` .15 左右、亮 `#fff` .2 左右），
  **不要写死灰色** —— 同一张图要在深灰褐（普通怪）、红（BOSS `#8A3223`）、蓝（主角 `--hero`）下都成立。
  ...
  斧刃别抬到 y<36，那是角的地盘。
- **地图上除了坛/箱/阶梯，全是画出来的**：人 `HERO`、泉 `SPRING`、商 `SHOP`、金币 `COIN`，
  怪走 `MOB_ART`（键是 `def.id`，跟 `content.js` 的 FOES/BOSS/GATEKEEPER 对齐）。
  ...
  **加新怪不画图也不会开天窗** —— `MOB_ART` 里找不到 id 就自动退回原来那个汉字。
  ...
  以及 `.c.mem.gold svg,.c.mem.feat svg,.c.mem.shop svg{opacity:.5}` 那条
  —— 那几张图里有写死的固定色（石头、描边），走出视野光调 `color` 淡不下来。
