...
- **复读者是「先欠着 + 5 秒补救」**（用户选的方案）：答错时伤害先记在 `B.pend` 上不扣，
  ...
  点了 → `B.rescue = true`，同一个词强制出成拼写题（`nextQuestion()` 里那个分支），拼对免伤、拼错结清；
  没点 → `closeRescue(true)` 自动结清。⚠️ **`openRescue()` 里那句 `wagerRow.hidden = true` 不能删** ——
  390×667 的小屏上不收起冒险那一行，5 秒倒计时按钮会被挤到屏幕外，等于没有。
  （顺带记一笔：`answer()` 里那个 `note` 变量**从来没被渲染过**，是老代码留下的死变量，别指望它显示。）
- **拼写题出现率是概率了**：普通怪基础 0%、BOSS 每 3 题一道；`spellRate()`（默诵 +30）按概率插进来；
  盲斗仍然是全拼写。拼写题不给「冒险」，所以默诵会顺带减少冒险次数。
